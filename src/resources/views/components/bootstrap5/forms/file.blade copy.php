...
$input_name=$name;
if($multiple && ! \Illuminate\Support\Str::endsWith($name, "[]")) $input_name.="[]";
    ...
    <div class="" id="file_container_{{\Illuminate\Support\Str::slug($name)}}">
        ...
        <div class="d-block d-md-flex ">
            <div class="col-file-input dropend" :class="colInputClass()">
                <label for="file_{{\Illuminate\Support\Str::slug($name)}}" class="file-btn mb-2"
                    ...
                    <template x-if="hasFiles()" x-cloak>
                        <div class="d-flex flex-column align-items-center justify-content-around">
                            ...
                            <i class="bi bi-list-task" x-show="multiple" @click.prevent.stop="showFiles()" data-bs-toggle="modal" data-bs-target="#file_detail_{{\Illuminate\Support\Str::slug($name??'')}}" title="Veure arxius"></i>
                        </div>
                    </template>
                ...
                <input class="form-control" x-ref="input" hidden type="file" x-on:change="setFiles" :accept="acceptedMimes()" id="file_{{\Illuminate\Support\Str::slug($name)}}" name="{{$input_name}}" :multiple="multiple">
            </div>
