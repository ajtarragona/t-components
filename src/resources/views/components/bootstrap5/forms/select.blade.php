@if (!$native)
    {{-- {{ $attributes }} --}}
    <div
        class="flex-grow-1 t-select {{ $overlayColor?('overlay-'.$overlayColor) : '' }}"
        :class="{ overlay: overlay, opened: open, 'with-search': search }"
    ...
    >
        ...
        <span
            class="dropdown {{$width=='fit-content'?'d-inline-block':'d-block'}}"
        ...
        >
            ...
            >
                ...
                <span
                    :class="{
                        'opacity-0 pe-none': !(allowClear && hasSelected()),
                        'ms-auto': width != 'fit-content',
                        'ms-2': width == 'fit-content',
                    }"
                    :visibility="{ hidden: !(allowClear && hasSelected()) }"
                    @click.prevent.stop="clearSelected"
                    class="text-reset z-10 align-self-end"
                    x-cloak
                    ><i class="bi bi-x"></i
                ></span>
            </button>
            ...
            >
                <div class="inner">
                    ...
                    <div class="relative overflow-y-auto options-container" :style="height ? 'max-height:' + height : ''">
                        <div
                            x-cloak
                            x-show="!isLoading && Object.values(options).length === 0"
                            class="opacity-75 small px-3 py-2"
                        ...
                        <div
                            x-intersect.margin.200px="await loadMore()"
                            class="relative opacity-75 small p-3"
                            x-show="isLoading || (lazyLoad && !allLoaded)"
                        >
